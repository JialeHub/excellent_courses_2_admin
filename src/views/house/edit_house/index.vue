<template>
  <card id="edit-house">
    <el-page-header @back="goBack" content="房源列表"></el-page-header>
    <el-form :model="form" :rules="rules" ref="Form" label-width="125px">
      ...
      <el-tabs v-model="active.three" type="card">
        <el-tab-pane label="项目介绍(中)" name="first">
          <custom-editor v-model="form.xmtx" :editor-key="2" mode="edit"/>
        </el-tab-pane>
        <el-tab-pane label="项目介绍(英)" name="second">
          <custom-editor v-model="form.xmtxEn" :editor-key="3" mode="edit"/>
        </el-tab-pane>
      </el-tabs>
      <el-tabs v-model="active.four" type="card">
        <el-tab-pane label="区域介绍(中)" name="first">
          <custom-editor v-model="form.zbss" :editor-key="4" mode="edit"/>
        </el-tab-pane>
        <el-tab-pane label="区域介绍(英)" name="second">
          <custom-editor v-model="form.zbssEn" :editor-key="5" mode="edit"/>
        </el-tab-pane>
      </el-tabs>
      <el-tabs v-model="active.five" type="card">
        <el-tab-pane label="购买流程(中)" name="first">
          <custom-editor v-model="form.gmlc" :editor-key="6" mode="edit"/>
        </el-tab-pane>
        <el-tab-pane label="购买流程(英)" name="second">
          <custom-editor v-model="form.gmlcEn" :editor-key="7" mode="edit"/>
        </el-tab-pane>
      </el-tabs>
      <el-tabs v-model="active.six" type="card">
        <el-tab-pane label="投资评估(中)" name="first">
          <custom-editor v-model="form.tzpg" :editor-key="8" mode="edit"/>
        </el-tab-pane>
        <el-tab-pane label="投资评估(英)" name="second">
          <custom-editor v-model="form.tzpgEn" :editor-key="9" mode="edit"/>
        </el-tab-pane>
      </el-tabs>
...
<script>
  import {getHouseDetailApi, editHouseApi} from '@/api/house';
  ...
  export default {
    ...
    methods: {
      getHouseDetail() {
        getHouseDetailApi(this.$route.query).then(result => {
          let response = result.data;
          response.objective = response.objectiveIds.split(',');
          response.objective = response.objective.map(item => parseInt(item));
          objectEvaluate(this.form, response);
          this.$refs.SearchSelectSeller.set(response.sellerId);
          this.$refs.SearchSelectCity.set(response.cityId);
        })
      },
      getOptions() {
      ...
      goBack() {
        this.$router.push({name: 'house_list'})
      }
    }
  }
</script>
